...
public class Manager extends HttpServlet {
	...
	public void managerLogin(HttpServletRequest request, HttpServletResponse response) 
		...
		managerForm.setName(new String(request.getParameter("name").getBytes("ISO-8859-1"),"UTF-8"));//获取管理员名称并设置name属性
	...
	// 查询管理员信息
	private void managerQuery(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String str = null;
	...
	private void managerAdd(HttpServletRequest request,HttpServletResponse response) 
		...
		managerForm.setName(new String(request.getParameter("name").getBytes("ISO-8859-1"),"UTF-8")); // 获取设置管理员名称
		managerForm.setPwd(request.getParameter("pwd")); // 获取并设置密码
		int ret = managerDAO.insert(managerForm); // 调用添加管理员信息
		if (ret == 1) {
			request.getRequestDispatcher("manager_ok.jsp?para=1").forward(request, response); // 转到管理员信息添加成功页面
		} else if (ret == 2) {
	...
	private void managerModify(HttpServletRequest request, HttpServletResponse response) 
		...
		managerForm.setName(new String(request.getParameter("name").getBytes("ISO-8859-1"),"UTF-8")); // 获取并设置管理员名称
		managerForm.setPwd(request.getParameter("pwd")); // 获取并设置管理员密码
		managerForm.setSysset(request.getParameter("sysset") == null ? 0 : Integer.parseInt(request.getParameter("sysset"))); // 获取并设置系统设置权限
		managerForm.setReaderset(request.getParameter("readerset") == null ? 0 : Integer.parseInt(request.getParameter("readerset"))); // 获取并设置读者管理权限
		managerForm.setBookset(request.getParameter("bookset") == null ? 0 : Integer.parseInt(request.getParameter("bookset"))); // 获取并设置图书管理权限
		managerForm.setBorrowback(request.getParameter("borrowback") == null ? 0 : Integer.parseInt(request.getParameter("borrowback"))); // 获取并设置图书借还权限
		managerForm.setSysquery(request.getParameter("sysquery") == null ? 0 : Integer.parseInt(request.getParameter("sysquery"))); // 获取并设置系统查询权限
		int ret = managerDAO.update(managerForm); // 调用设置管理员权限的方法
		if (ret == 0) {
			request.setAttribute("error", "设置管理员权限失败！"); // 保存错误提示信息到error参数中
			request.getRequestDispatcher("error.jsp").forward(request, response); // 转到错误提示页面
		} else {
			request.getRequestDispatcher("manager_ok.jsp?para=2").forward(request, response);// 转到权限设置成功页面
		}
	}
	...
	private void managerDel(HttpServletRequest request, HttpServletResponse response) 
		...
		if (ret == 0) {
			...
			request.getRequestDispatcher("manager_ok.jsp?para=3").forward(request, response); // 转到删除管理员信息成功页面
		}
	}
